...
import { GET } from '../search-router/route';
...
jest.mock('../elasticsearch/search/keyword-search/route');
jest.mock('../elasticsearch/search/semantic-search/route');
jest.mock('../elasticsearch/search/combined-search/route');
...
describe('Search Router', () => {
  ...
  it('should handle semantic search for videos', async () => {
    ...
    const request = new NextRequest(
      new URL('http://localhost:3000/api/search?query=temple+architecture+analysis')
    );

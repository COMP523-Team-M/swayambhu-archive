  ...
  try {
    ...
    if (transcriptUpdates && transcriptUpdates.length > 0) {
      ...
      for (const update of transcriptUpdates) {
        transcriptJson.results[update.segmentIndex].alternatives[0].transcript = update.newTranscript;
      }
      ...
      const englishTranscriptJson: TranscriptJson = {
        results: await Promise.all(transcriptJson.results.map(async (result: TranscriptResult) => ({
          alternatives: [{
      ...
      for (let i = 0; i < transcriptJson.results.length; i++) {
        ...
        const lastWord = result.alternatives[0].words[result.alternatives[0].words.length - 1];
        const startTime = convertTimeToSeconds(firstWord.startOffset || firstWord.endOffset);

...
function addTypeToResults(results: any[], type: 'video' | 'snippet'): SearchResult[] {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    switch (searchType) {
      ...
      case 'semantic':
        if (!queryEmbedding) throw new Error('Missing query embedding for semantic search');
      ...
      case 'combined':
      default:
        if (!queryEmbedding) throw new Error('Missing query embedding for combined search');
        ...
        searchResults = addTypeToResults(combinedResults, level === 'video' ? 'video' : 'snippet');
        break;
    }

...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    try {
      ...
      // Step 2: Insert metadata into the 'videos' index using the ingest pipeline
      await client.index({
        index: 'videos',
        id: vidID,
        pipeline: 'embedding-pipeline', // Use the ingest pipeline
        body: {
          vidID,
          vidDescription,
          uploadDate,
      ...
      // Step 4: Add each transcript snippet to the 'video_snippets' index using the ingest pipeline

          ...
          <div className="relative h-[135px] w-[240px]">
            ...
            {isSnippet && result.timeSegment && (
              <div className="absolute bottom-2 right-2 flex items-center gap-1 rounded-full bg-black/70 px-2 py-1 text-xs text-white backdrop-blur-sm">
                ...
                <span>
                  {Math.floor(result.timeSegment / 60)}:
                  {(result.timeSegment % 60).toString().padStart(2, "0")}
                </span>
              </div>
            )}
          </div>
...
export default function SearchResultPage() {
  ...
  const getList = async () => {
    ...
    try {
      ...
      state.totalPages = Math.ceil(
        data.metadata.totalResults / state.itemsPerPage,
      );
    } catch (error) {
  ...
  const handleSearchClick = () => {
    if (state.searchQuery.trim()) {
      router.push(
        `/searchResults?query=${encodeURIComponent(state.searchQuery)}`,
      );
    }
  };
